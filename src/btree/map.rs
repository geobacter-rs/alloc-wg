...
use super::{
    ...
    node::{self, marker, ForceResult::*, Handle, InsertResult::*, NodeRef},
...
impl<K: Clone, V: Clone, A: Clone + AllocRef> Clone for BTreeMap<K, V, A> {
    fn clone(&self) -> BTreeMap<K, V, A> {
            ...
            match node.force() {
                Leaf(leaf) => {
                    let mut out_tree = BTreeMap {
                        root: Some(node::Root::new_leaf(&mut alloc)),
                        length: 0,
                ...
                Internal(internal) => {
                    ...
                    {
                        let out_root = BTreeMap::ensure_is_owned(&mut out_tree.root, &mut alloc);
                        ...
                        while let Ok(kv) = in_edge.right_kv() {
                            ...
                            out_node.push(
                                k,
                                v,
                                subroot.unwrap_or_else(|| node::Root::new_leaf(&mut alloc)),
                            );
        ...
        if self.is_empty() {
            ...
            clone_subtree(
                self.root.as_ref().unwrap().node_as_ref(),
                ManuallyDrop::into_inner(self.alloc.clone()),
    ...
    Q: Ord,
{
    ...
    fn replace(&mut self, key: K) -> Option<K> {
        let (map, dormant_map) = DormantMutRef::new(self);
        let root_node = Self::ensure_is_owned(&mut map.root, &mut *map.alloc).node_as_mut();
...
#[derive(Debug)]
pub struct IntoKeys<K, V, A: AllocRef> {
...
//#[unstable(feature = "map_into_keys_values", issue = "75294")]
#[derive(Debug)]
pub struct IntoValues<K, V, A: AllocRef> {
    inner: IntoIter<K, V, A>,
}
...
pub enum Entry<'a, K: 'a, V: 'a, A: AllocRef> {
    ...
    //#[stable(feature = "rust1", since = "1.0.0")]
    Vacant(
        /* #[stable(feature = "rust1", since = "1.0.0")] */ VacantEntry<'a, K, V, A>,
    ),

    /// An occupied entry.
    //#[stable(feature = "rust1", since = "1.0.0")]
    Occupied(
        /* #[stable(feature = "rust1", since = "1.0.0")] */ OccupiedEntry<'a, K, V, A>,
    ),
}
...
impl<K: Ord, V, A: AllocRef> BTreeMap<K, V, A> {
    ...
    fn from_sorted_iter<I: Iterator<Item = (K, V)>>(&mut self, iter: I) {
        let root = Self::ensure_is_owned(&mut self.root, &mut *self.alloc);
        ...
        for (key, value) in iter {
            ...
            if cur_node.len() < node::CAPACITY {
                ...
                loop {
                    match test_node.ascend() {
                        ...
                        Err(_) => {
                            ...
                            open_node = root.push_internal_level(&mut *self.alloc);
                            break;
                        }
                    }
                }
                ...
                let mut right_tree = node::Root::new_leaf(&mut *self.alloc);
                for _ in 0..tree_height {
                    right_tree.push_internal_level(&mut *self.alloc);
                }
        ...
        A: Clone,
    {
        ...
        let right_root = Self::ensure_is_owned(&mut right.root, &mut *self.alloc);
        for _ in 0..left_root.height() {
            right_root.push_internal_level(&mut *self.alloc);
        }
        ...
        left_root.fix_right_border(&mut *self.alloc);
        right_root.fix_left_border(&mut *self.alloc);
...
//#[stable(feature = "rust1", since = "1.0.0")]
impl<'a, K: 'a, V: 'a, A: AllocRef> IntoIterator for &'a BTreeMap<K, V, A> {
    type Item = (&'a K, &'a V);
...
//#[stable(feature = "rust1", since = "1.0.0")]
impl<'a, K: 'a, V: 'a, A: AllocRef> IntoIterator for &'a mut BTreeMap<K, V, A> {
    ...
    fn drop(&mut self) {
        ...
        impl<'a, K, V, A: AllocRef> Drop for DropGuard<'a, K, V, A> {
            fn drop(&mut self) {
                ...
                unsafe {
                    let mut node = unwrap_unchecked(ptr::read(&self.0.front))
                        .into_node()
                        .forget_type();
        ...
        unsafe {
            if let Some(front) = ptr::read(&self.front) {
                ...
                while let Some(parent) = node.deallocate_and_ascend(&mut self.alloc) {
                    node = parent.into_node().forget_type();
                }
            }
        }
    ...
    F: FnMut(&K, &mut V) -> bool,
{
    ...
    fn next(&mut self) -> Option<(K, V)> {
        self.inner.next(&mut self.pred, &mut self.alloc)
    }
...
impl<'a, K: Ord, V, A: AllocRef> OccupiedEntry<'a, K, V, A> {
    ...
    fn remove_kv(self) -> (K, V) {
        ...
        let (old_kv, _) = self
            .handle
            .remove_kv_tracking(|_| emptied_internal_root = true, self.alloc);
        // SAFETY: we consumed the intermediate root borrow, `self.handle`.
        let map = unsafe { self.dormant_map.awaken() };
        map.length -= 1;
        if emptied_internal_root {
            let root = map.root.as_mut().unwrap();
            root.pop_internal_level(&mut *map.alloc);
        }
        old_kv
    }
}
...
fn handle_underfull_node<'a, K: 'a, V: 'a, A: AllocRef>(
    ...
    if handle.can_merge() {
        let offset = if is_left {
            handle.reborrow().left_edge().descend().len() + 1
        } else {
            0
        };
        Merged(handle.merge(alloc), is_left, offset)
    } else {
        if is_left {
            handle.steal_left();
        } else {
